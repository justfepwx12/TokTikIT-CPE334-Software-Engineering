...
describe("StaffTicketDetail", () => {
  beforeEach(() => {
    ...
    // Communication panels (Issue 7) fetch inside the detail screen.
  ...
  it("mounts both communication panels for staff (AC-22/AC-23)", async () => {
    renderDetail();
    await waitFor(() => expect(screen.getByTestId("staff-ticket-detail")).toBeDefined());

    expect(screen.getByTestId("public-comments")).toBeDefined();
    expect(screen.getByTestId("internal-notes")).toBeDefined();
    await waitFor(() => expect(api.getComments).toHaveBeenCalledWith(42));
    await waitFor(() => expect(api.getNotes).toHaveBeenCalledWith(42));
  });
});
