...
const ADMIN_EMAIL = "l3-adm-root@toktikit.com";
const ADMIN2_EMAIL = "l3-adm-second@toktikit.com";
const STAFF_EMAIL = "l3-adm-staff@toktikit.com";
const REQ_EMAIL = "l3-adm-req@toktikit.com";
...
describe("Lab 3 Admin User Management API (AC-26–AC-31)", () => {
  let adminId = 0;
  let requesterId = 0;
  ...
  it("lists users with search and role filter for admins (AC-26)", async () => {
    ...
    const search = await agent.get("/api/admin/users").query({ search: "l3-adm-req" });
  ...
  it("creates users with mustChangePassword=true and rejects duplicates (AC-26/27)", async () => {
    ...
    const create = await agent.post("/api/admin/users").send({
      name: "Newbie Staff",
      email: "l3-adm-newbie@toktikit.com",
      role: "IT_STAFF",
    ...
    try {
      ...
      const mcp = await agent.post("/api/admin/users").send({
        name: "Sneaky",
        email: "l3-adm-sneaky@toktikit.com",
        role: "REQUESTER",
        password: "TempPass123!",
      ...
      await prisma.user.deleteMany({ where: { email: { in: ["l3-adm-newbie@toktikit.com", "l3-adm-sneaky@toktikit.com"] } } });
    }
  });
  ...
  it("serializes concurrent last-admin removals: one wins, one gets 400 (BR-11 race)", async (ctx) => {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      const a = await mkUser("Adm Race A", "l3-adm-race-a@toktikit.com", "ADMIN");
      const b = await mkUser("Adm Race B", "l3-adm-race-b@toktikit.com", "ADMIN");
      const deactivated: number[] = [];
      try {
        const agentA = await loginAs("l3-adm-race-a@toktikit.com", TEST_PASSWORD);
        const agentB = await loginAs("l3-adm-race-b@toktikit.com", TEST_PASSWORD);
        ...
        await prisma.user.deleteMany({ where: { email: { in: ["l3-adm-race-a@toktikit.com", "l3-adm-race-b@toktikit.com"] } } });
      }
