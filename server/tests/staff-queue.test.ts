...
const STAFF_EMAIL = "queue-staff@toktikit.com";
const ADMIN_EMAIL = "queue-admin@toktikit.com";
const REQ_A_EMAIL = "queue-req-a@toktikit.com";
const REQ_B_EMAIL = "queue-req-b@toktikit.com";
...
describe("GET /api/staff/tickets (api-spec §2)", () => {
  beforeAll(async () => {
  ...
  it("lets IT_STAFF and ADMIN see all requesters' tickets with full shape", async () => {
    for (const email of [STAFF_EMAIL, ADMIN_EMAIL]) {
      ...
      const mine = res.body.tickets.filter((t: { title: string }) =>
        String(t.title).startsWith("Queue ")
      );
  ...
  it("searches case-insensitively across title and description", async () => {
    ...
    expect(titles).toEqual(expect.arrayContaining(["Queue Alpha VPN Down", "Queue Gamma VPN Flaky"]));
  ...
  it("filters by status and by IT priority", async () => {
    ...
    expect(
      byStatus.body.tickets.some((t: { title: string }) => t.title === "Queue Alpha VPN Down")
    ).toBe(true);
